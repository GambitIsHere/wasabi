...
- **Admin gate:** in-app middleware, Google SSO via Auth.js v5 with domain allowlist (`AUTH_ALLOWED_EMAIL_DOMAIN`). Vercel's Deployment Protection is OFF so the public API stays reachable; the middleware re-asserts the gate on every non-public path.
